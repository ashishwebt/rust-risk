...
impl Default for AppState {
    fn default() -> Self {
        ...
        let mut vol_surface = VolSurface::new(vec![90.0, 95.0, 100.0, 105.0, 110.0], vec![0.083, 0.25, 0.5, 1.0]);
...
impl AppState {
    /// Drains every pending event from the feed channel. This loop is
    /// IDENTICAL regardless of whether the active adapter is push-based
    /// (mock/websocket) or pull-based (Yahoo polling) -- that's the whole
    /// point of normalizing everything into `FeedEvent` upstream.
    pub fn pump_feed(&mut self) {
        let rx = self.feed_manager.receiver();
        while let Ok(event) = rx.try_recv() {
            match event {
                FeedEvent::Tick(tick) => self.apply_tick(tick),
                FeedEvent::Status(status) => self.feed_status = status,
            }
        }
    }
    ...
    pub fn portfolio_parametric_var(&self) -> f64 {
        let portfolio_value: f64 = self
            ...
            .sum();
        // Rough proxy: use average position volatility as the portfolio's
        // daily vol input for the parametric estimate.
...
fn seed_vol_surface(surface: &mut VolSurface) {
    // A simple synthetic smile/term structure: vol rises away from ATM and
    // is higher for shorter expiries (typical equity skew shape).
    let base = 0.20;
